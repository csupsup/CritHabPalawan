...
sat.img <- list.files("images/tile01/", pattern = "tif", full.names=TRUE)
#read admin boundary
adm <- st_read("adm/balabac_t1_adm_utm.shp", quiet = TRUE)
...
```
```{r, ndvi, fig.width = 4, fig.height = 4}
#plot NDVI
...
training_points <- st_read("rois/balabact1_roi_centroids.shp", quiet = TRUE)
...
profiles <- df %>% 
  ...
  na.omit() %>% 
  group_by(id) %>% 
  summarise(balabact1_b10 = mean(balabact1_b10),
            balabact1_b11 = mean(balabact1_b11),
            balabact1_b12 = mean(balabact1_b12),
            balabact1_b2 = mean(balabact1_b2),
            balabact1_b3 = mean(balabact1_b3),
            balabact1_b4 = mean(balabact1_b4),
            balabact1_b5 = mean(balabact1_b5),
            balabact1_b6 = mean(balabact1_b6),
            balabact1_b7 = mean(balabact1_b7),
            balabact1_b8 = mean(balabact1_b8),
            balabact1_b8a = mean(balabact1_b8a)) %>% 
...
#export tif file
writeRaster(veg.pr, "balabac_t1_veg2019.tif", overwrite=TRUE)
