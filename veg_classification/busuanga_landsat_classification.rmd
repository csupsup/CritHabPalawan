...
sat.img <- list.files("images/", pattern = "tif", full.names=TRUE)

#read admin boundary
adm <- st_read("adm/busuanga_adm_utm.shp", quiet = TRUE)
...
#plot true color composite: 4-3-2
par(col.axis="white",col.lab="white",tck=0)
plotRGB(stack.img, r = 4, g = 3, b = 2, axes = TRUE, 
        stretch = "lin", main = "True color composite")
box(col="white")
#plot false color composite: 5-4-3
par(col.axis="white",col.lab="white",tck=0)
plotRGB(stack.img, r = 5, g = 4, b = 3, axes = TRUE, stretch = "lin", main = "False color composite")
box(col="white")

#calculate NDVI
ndvi <- (stack.img[[5]] - stack.img[[4]])/(stack.img[[5]] + stack.img[[4]])
...
training_points <- st_read("rois/busuanga_roi_centroids.shp", quiet = TRUE)
...
profiles <- df %>% 
  ...
  na.omit() %>% 
  group_by(id) %>% 
  summarise(band.1.surface.reflectance = mean(band.1.surface.reflectance),
            band.2.surface.reflectance = mean(band.2.surface.reflectance),
            band.3.surface.reflectance = mean(band.3.surface.reflectance),
            band.4.surface.reflectance = mean(band.4.surface.reflectance),
            band.5.surface.reflectance = mean(band.5.surface.reflectance),
            band.6.surface.reflectance = mean(band.6.surface.reflectance),
            band.7.surface.reflectance = mean(band.7.surface.reflectance)) %>% 
...
head(profiles)
#rename colnames
colnames(profiles)[2] <- "band1"
colnames(profiles)[3] <- "band2"
colnames(profiles)[4] <- "band3"
colnames(profiles)[5] <- "band4"
colnames(profiles)[6] <- "band5"
colnames(profiles)[7] <- "band6"
colnames(profiles)[8] <- "band7"

profiles %>% 
  select(-id) %>% 
  gather() %>% 
  mutate(class = rep(c("Pristine Forest", "Secondary Growth", "Open Area/Cultivation", "Mangrove"), 7)) %>% 
...
#export tif file
writeRaster(veg.pr, "busuanga_veg2019.tif", overwrite=TRUE)
